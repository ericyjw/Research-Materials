...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ec.exportFile(new File("real_corr.png"));
            ec.exportFile(new File("real_corr.pdf"));
            ec.exportFile(new File("real_corr.svg"));
    ...
    private static void identifyClusters(GraphModel graphModel, AppearanceController appearanceController,
        ...
        Color[] preset = new Color[]{new Color(0x4788FF),
            new Color(0xFF3F29), new Color(0x3BFF9A), new Color(0x42EFFF), new Color(0xFFC951)};
        Color[] chosen = new Color[partition.size()];
        if (partition.size() >= 0) System.arraycopy(preset, 0, chosen, 0, partition.size());
        Palette palette = new Palette(chosen);
